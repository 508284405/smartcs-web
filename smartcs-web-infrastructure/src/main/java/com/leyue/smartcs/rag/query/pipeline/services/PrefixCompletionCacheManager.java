...
public class PrefixCompletionCacheManager {
     ...
     * 缓存条目
     */
    private record CacheEntry(List<String> results, long timestamp, long accessCount) {
        public CacheEntry(List<String> results) {
            this(new ArrayList<>(results), System.currentTimeMillis(), 1);
        }
    ...
    private void evictL1Cache() {
        ...
        String evictKey = l1Cache.entrySet().stream()
                .min(Comparator.comparing(entry -> entry.getValue().accessCount()))
