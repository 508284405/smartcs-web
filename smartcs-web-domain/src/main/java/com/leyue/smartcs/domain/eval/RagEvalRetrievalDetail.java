...
@AllArgsConstructor
@NoArgsConstructor
public class RagEvalRetrievalDetail {
     ...
     * 检索候选结果
     */
    private List<RetrievalCandidate> retrievalCandidates;
     ...
     * 最终选择的上下文
     */
    private List<ContextFragment> finalContexts;
    
    /**
     * 过滤掉的候选项
     */
    private List<RetrievalCandidate> filteredCandidates;
     ...
     * 检查是否有重排序结果
     */
    public boolean hasRerankResults() {
        return rerankCandidates != null && !rerankCandidates.isEmpty();
    }
    ...
    public boolean hasRetrievalResults() {
        return retrievalCandidates != null && !retrievalCandidates.isEmpty();
    }
    
    /**
     * 获取有效的候选结果（优先返回重排序结果）
     */
    public List<RetrievalCandidate> getEffectiveCandidates() {
        if (hasRerankResults()) {
     ...
     * 计算检索延迟占比
     */
    public Double getRetrievalLatencyRatio() {
        if (retrievalLatencyMs == null || rerankLatencyMs == null) {
            return null;
        }
        int totalLatency = retrievalLatencyMs + rerankLatencyMs;
        return totalLatency > 0 ? (double) retrievalLatencyMs / totalLatency : null;
    }
